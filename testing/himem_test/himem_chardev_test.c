 ...
 * Data check function
 ****************************************************************************/
void himem_data_check(uint8_t *data1,uint8_t *data2,uint32_t len, uint32_t offset){
	uint32_t ok=0;
	uint32_t ng=0;
 ...
 ****************************************************************************/
void himem_multi_dev_check(uint32_t size, uint32_t offset)
{
	int fd1;
	int fd2;
	...
	//read dev1
	memset(work_buffer2, 0, sizeof(work_buffer2));
	if(lseek(fd1, offset, SEEK_SET) < 0){
		printf("HiMem Seek failed. \n");
		goto common_exit;
	}
 ...
 ****************************************************************************/
void himem_check(uint32_t size, uint32_t offset)
{
	int fd;
	uint8_t               *work_buffer1 = NULL;
	uint8_t               *work_buffer2 = NULL;
...
int himem_chardev_test(void)
{
	...
	for(int i=0; i<4; i++){			/*1M--->4M*/
		printf("\nHimem Cdev test: %d MB\n", (i+1));
		for(int j=0; j<11; j++){	/*1K--->1M*/
			himem_check(1024*pow(2, j), 1048576*i);
		}
	}   

	printf("\n\nRead and write multiple device files!\n");
	himem_multi_dev_check(16384, 0); /*16K*/
	himem_multi_dev_check(1048576, 0); /*1MB*/
